  ...
  const handleAddItem = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/recipes/${itemForm.recipe_id}/items`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
                ...
                <form onSubmit={handleCreateIngredient} className="space-y-4">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => setIngredientForm({ ...ingredientForm, default_unit: e.target.value })}
                      placeholder="e.g., g, ml, kg"
                      required
                    />
                  </div>
                  <div className="flex gap-2">
                    <Button type="button" variant="outline" onClick={() => setOpenIngredientDialog(false)}>
                      Cancel
                    </Button>
